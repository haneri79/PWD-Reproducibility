...
mkdir -p "$LOGDIR"
PWD_SOLVER=${PWD_SOLVER:-pwd_solver}
...
run() {
  ...
  ts_plan=$(date +%s.%N)
  $PWD_SOLVER --make-plans --config "$CONF" >/dev/null 2>&1 || true
  te_plan=$(date +%s.%N)
  ...
  ( while true; do
      ...
      echo "rapl_uj_ts=${ts} rapl_uj=${e}" >> "$LOGDIR/.pow_pwd_${tag}.tmp"
      sleep 1
    done ) & EM_PID=$!
  ts=$(date +%s.%N)
  /usr/bin/time -v $PWD_SOLVER --config "$CONF" 2> "$LOGDIR/.time_pwd_${tag}.tmp" | tee -a "$log"
  te=$(date +%s.%N)
  kill $EM_PID || true
  awk -v a="$ts" -v b="$te" 'BEGIN{printf "wall_time_s=%.6f\n", (b-a)}' >> "$log"
  awk '/Maximum resident set size/ {printf "peak_ram_gb=%.3f\n", $6/1024/1024}' "$LOGDIR/.time_pwd_${tag}.tmp" >> "$log"
  python - <<'PY' >>"$log"
import glob
uj=0.0
for f in glob.glob("hero/HS1_softCoulomb_3D/logs/.pow_pwd_*.tmp"):
    with open(f) as h:
        for ln in h:
            if "rapl_uj=" in ln:
